...
let config = new botframework_config_1.BotConfiguration();
...
async function processConfiguration() {
    ...
    if (!Object.values(utils_1.RegionCodes).find((r) => args.location == r)) {
    ...
    try {
        ...
        if (!args.quiet) {
            ...
            for (let resource of recipe.resources) {
                switch (resource.type) {
                    case botframework_config_1.ServiceTypes.AppInsights:
                        rows.push([`Azure AppInsights Service`, `${utils_1.regionToAppInsightRegionNameMap[args.location]}`, `F0`, args.groupName]);
                        break;
                    ...
                    case botframework_config_1.ServiceTypes.Luis:
                        ...
                        if (!args.luisPublishRegion) {
                            args.luisPublishRegion = utils_1.luisPublishRegions.find((value) => value == args.location);
                            if (!args.luisPublishRegion) {
                                args.luisPublishRegion = utils_1.regionToLuisPublishRegionMap[args.location];
                            }
                        }
                        if (resource.type == botframework_config_1.ServiceTypes.Dispatch)
                    ...
                    case botframework_config_1.ServiceTypes.QnA:
                        ...
                        rows.push([`Azure Search Service`, `${utils_1.regionToSearchRegionMap[args.location]}`, `Standard`, args.groupName]);
                        break;
                    default:
                        break;
                }
            ...
            for (let row of rows)
                table.push(row);
            await stdioAsync_1.logAsync(table.toString());
            if (!args.force) {
        ...
        let azSitePlan;
        let storageInfo;
        let azQnaSubscription;
        let azLuisSubscription;
        let azAppInsights;
        let azBotExtended;
        let azBotEndpoint;
        ...
        for (let resource of recipe.resources) {
            if (resource.type == botframework_config_1.ServiceTypes.Bot) {
                if (!azBot) {
                    azBot = await createBot();
                    azBotEndpoint = azBot;
                    azBotExtended = await runCommand(`az bot show -g ${args.groupName} -n ${args.name}`, `Fetching bot extended information [${args.name}]`);
                    // fetch co-created resources so we can get blob and appinsights data
                    let azGroupResources = await runCommand(`az resource list -g ${azGroup.name}`, `Fetching co-created resources [${args.name}]`);
                    let botWebSite;
                    ...
                    if (botWebSite) {
                        let botAppSettings = await runCommand(`az webapp config appsettings list -g ${args.groupName} -n ${botWebSite.name}`, `Fetching bot website appsettings [${args.name}]`);
                        ...
                        if (!args.secret) {
                            ...
                            // set the appsetting
                            await runCommand(`az webapp config appsettings set -g ${args.groupName} -n ${botWebSite.name} --settings botFileSecret="${args.secret}" `, `Setting bot website appsettings secret [${args.name}]`);
                        }
                    }
                    else {
                        throw new Error('botsite was not found');
                    }
                    config.services.push(new botframework_config_1.BotService({
                        type: botframework_config_1.ServiceTypes.Bot,
                        id: resource.id,
                        name: azBot.name,
            ...
            switch (resource.type) {
                ...
                case botframework_config_1.ServiceTypes.Dispatch:
                    if (!azLuisSubscription) {
                        if (!args.luisAuthoringRegion) {
                            if (utils_1.regionToLuisAuthoringRegionMap.hasOwnProperty(args.location))
                                args.luisAuthoringRegion = utils_1.regionToLuisAuthoringRegionMap[args.location];
                            else
                        ...
                        if (!args.luisPublishRegion) {
                            args.luisPublishRegion = utils_1.luisPublishRegions.find((value) => value == args.location);
                            if (!args.luisPublishRegion) {
                                args.luisPublishRegion = utils_1.regionToLuisAuthoringRegionMap[args.location];
                            }
                        }
                        // create luis subscription
                        let luisCogsName = `${args.name}-LUIS`;
                        azLuisSubscription = await runCommand(`az cognitiveservices account create -g ${azGroup.name} --kind LUIS -n "${luisCogsName}" --location ${args.luisPublishRegion} --sku S0 --yes`, `Creating LUIS Cognitive Service [${luisCogsName}]`);
                        // get keys
                        let luisKeys = await runCommand(`az cognitiveservices account keys list -g ${azGroup.name} -n "${luisCogsName}"`, `Fetching LUIS Keys [${luisCogsName}]`);
                ...
                case botframework_config_1.ServiceTypes.QnA:
                    if (!azQnaSubscription) {
                        if (!azSitePlan) {
                            azSitePlan = await runCommand(`az appservice plan create -g  ${args.groupName} --sku s1 --name ${args.name}`, `Creating site plan [${args.name}]`);
                        }
                        ...
                        let searchResult = await runCommand(`az search service create -g ${azGroup.name} -n "${searchName}" --location ${utils_1.regionToSearchRegionMap[args.location]} --sku standard`, `Creating Azure Search Service [${searchName}]`);
                        // get search keys
                        let searchKeys = await runCommand(`az search admin-key show -g ${azGroup.name} --service-name "${searchName}"`, `Fetching Azure Search Service keys [${searchName}]`);
                        // create qna host service
                        let qnaHostName = args.name + '-qnahost';
                        let createQnaWeb = await runCommand(`az webapp create -g ${azGroup.name} -n ${qnaHostName} --plan ${args.name}`, `Creating QnA Maker host web service [${qnaHostName}]`);
                        ...
                        await runCommand(`az webapp cors add -g ${azGroup.name} -n ${qnaHostName} -a "*"`, `Configuring QnA Maker host web service CORS [${qnaHostName}]`);
                        ...
                        let azQnaKeys = await runCommand(`az cognitiveservices account keys list -g ${azGroup.name} -n "${qnaAccountName}"`, `Fetching QnA Maker Cognitive Service [${qnaAccountName}]`);
        ...
        for (let resource of recipe.resources) {
            switch (resource.type) {
                ...
                case botframework_config_1.ServiceTypes.BlobStorage:
                    {
                        ...
                        if (!storageInfo) {
                            let storageName = `${azGroup.name.toLowerCase() + generateShortId()}storage`;
                            storageInfo = await runCommand(`az storage account create -g ${azGroup.name} -n "${storageName}" --location ${args.location} --sku Standard_LRS`, `Creating Azure Blob Storage  [${storageName}]`);
                        }
                        if (storageInfo) {
                            let blobConnection = await runCommand(`az storage account show-connection-string -g ${azGroup.name} -n "${storageInfo.name}"`, `Fetching Azure Blob Storage connection string [${args.name}]`);
                            let blobResource = resource;
                            config.services.push(new botframework_config_1.BlobStorageService({
                ...
                case botframework_config_1.ServiceTypes.CosmosDB:
                    {
                        let cosmosResource = resource;
                        let cosmosName = `${args.name.toLowerCase()}`;
                        // az cosmosdb create --n name -g Group1
                        let cosmosDb = await runCommand(`az cosmosdb create -n ${cosmosName} -g ${azGroup.name}`, `Creating Azure CosmosDB account [${cosmosName}] ${chalk.default.italic.yellow(`(Please be patient, this may take 5 minutes)`)}`);
                        // get keys
                        let cosmosDbKeys = await runCommand(`az cosmosdb list-keys -g ${azGroup.name} -n ${cosmosName}`, `Fetching Azure CosmosDB account keys [${args.name}]`);
                        ...
                        await runCommand(`az cosmosdb database create -g ${azGroup.name} -n ${cosmosName} --key ${cosmosDbKeys.primaryMasterKey} -d ${cosmosResource.database} --url-connection https://${cosmosName}.documents.azure.com:443/`, `Creating Azure CosmosDB database [${cosmosResource.database}]`);
                        ...
                        await runCommand(`az cosmosdb collection create -g ${azGroup.name} -n ${cosmosName} --key ${cosmosDbKeys.primaryMasterKey} -d ${cosmosResource.database} --url-connection https://${cosmosName}.documents.azure.com:443/ --collection-name ${cosmosResource.collection}`, `Creating Azure CosmosDB collection [${cosmosResource.collection}]`);
                        ...
                        config.services.push(new botframework_config_1.CosmosDbService({
                ...
                case botframework_config_1.ServiceTypes.Endpoint:
                    {
                        ...
                        if (urlResource.url && urlResource.url.indexOf('localhost') > 0) {
                            ...
                            config.services.push(new botframework_config_1.EndpointService({
                ...
                case botframework_config_1.ServiceTypes.File:
                    {
                        ...
                        config.services.push(new botframework_config_1.FileService({
                ...
                case botframework_config_1.ServiceTypes.Generic:
                    {
                        let genericResource = resource;
                        config.services.push(new botframework_config_1.GenericService({
                ...
                case botframework_config_1.ServiceTypes.Dispatch:
                    {
                        ...
                        let dispatchResource = resource;
                        let dispatchService = Object.assign({ serviceIds: dispatchResource.serviceIds, }, luisService);
                        dispatchService.type = botframework_config_1.ServiceTypes.Dispatch;
                        dispatchService.id = resource.id; // keep same resource id
                        config.services.push(new botframework_config_1.DispatchService(dispatchService));
                        await config.save();
                    }
                    break;
                case botframework_config_1.ServiceTypes.Luis:
                ...
                case botframework_config_1.ServiceTypes.QnA:
                    {
                        ...
                        let svc = await runCommand(`qnamaker create kb --subscriptionKey ${args.qnaSubscriptionKey} --name "${kbName}" --in ${qnaPath} --wait --msbot -q`, `Creating QnA Maker KB [${kbName}]`);
                        let service = new botframework_config_1.QnaMakerService(svc);
                        ...
                        // publish  
                        try {
                            await runCommand(`qnamaker publish kb --subscriptionKey ${service.subscriptionKey} --kbId ${service.kbId} --hostname ${service.hostname} --endpointKey ${service.endpointKey}`, `Publishing QnA Maker KB [${kbName}]`);
                        }
                        catch (err) {
        ...
        if (azBot) {
            ...
            if (!hasBot && azBotEndpoint) {
                ...
                config.connectService(new botframework_config_1.BotService({
                    name: azBot.name,
                    tenantId: args.tenantId,
                    subscriptionId: args.subscriptionId,
                    resourceGroup: args.groupName,
                ...
                // add endpoint
                config.connectService(new botframework_config_1.EndpointService({
                    type: botframework_config_1.ServiceTypes.Endpoint,
                    name: azBot.name,
                    appId: azBotEndpoint.appId,
                    appPassword: azBotEndpoint.appPassword,
            ...
            if (!hasBlob && storageInfo) {
                // this was created via az bot create, get the connection string and then hook it up
                let blobConnection = await runCommand(`az storage account show-connection-string -g ${azGroup.name} -n "${storageInfo.name}"`, `Fetching storage connection string [${storageInfo.name}]`);
                config.connectService(new botframework_config_1.BlobStorageService({
                    name: storageInfo.name,
                    serviceName: storageInfo.name,
                    tenantId: args.tenantId,
                    subscriptionId: args.subscriptionId,
...
async function getAppInsightsService(azAppInsights) {
    ...
    return new botframework_config_1.AppInsightsService({
        type: botframework_config_1.ServiceTypes.AppInsights,
        tenantId: args.tenantId,
        subscriptionId: args.subscriptionId,
        resourceGroup: args.groupName,
...
async function importAndTrainLuisApp(luisResource) {
    let luisPath = path.join(args.folder, `${luisResource.id}.luis`);
    let luisService;
    const luisAuthoringRegion = utils_1.regionToLuisAuthoringRegionMap[args.location];
    let luisAppName = `${args.name}_${luisResource.name}`;
    let svcOut = await runCommand(`luis import application --region ${luisAuthoringRegion} --appName "${luisAppName}" --in ${luisPath} --authoringKey ${args.luisAuthoringKey} --msbot`, `Creating and importing LUIS application [${luisAppName}]`);
    luisService = new botframework_config_1.LuisService(svcOut);
    // mark application as public (TEMPORARY, THIS SHOULD BE REMOVED ONCE LUIS PROVIDES KEY ASSIGN API)
    await runCommand(`luis update settings --region ${luisAuthoringRegion} --appId ${luisService.appId} --authoringKey ${luisService.authoringKey} --public true`, `Updating LUIS settings [${luisService.name}]`);
    try {
        ...
        await processUtils_1.spawnAsync(command, (out) => process.stdout.write(out), (err) => process.stderr.write(err));
        // publish application
        await runCommand(`luis publish version --region ${luisAuthoringRegion} --appId ${luisService.appId} --authoringKey ${luisService.authoringKey} --versionId "${luisService.version}" --publishRegion ${args.luisPublishRegion} `, `Publishing LUIS application [${luisService.name}] to ${args.luisPublishRegion}`);
    }
    catch (err) {
...
async function createBot() {
    args.insightsRegion = args.insightsRegion || utils_1.regionToAppInsightRegionNameMap[args.location];
    ...
    let stdout = await processUtils_1.spawnAsync(command, undefined, (stderr) => {
    ...
    let botService = JSON.parse(stdout);
    return botService;
}
async function createGroup() {
    ...
    let azGroup = await runCommand(`az group create -g ${args.groupName} -l ${args.location}`, `Creating Azure group [${args.groupName}]`);
...
function logCommand(args, message, command) {
    if (!args.quiet) {
...
//# sourceMappingURL=msbot-clone-services.js.map