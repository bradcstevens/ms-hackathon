...
if (!args.quiet) {
    ...
    while (!args.endpoint || args.endpoint.length === 0) {
        ...
        args.endpoint = readline.question(`What localhost endpoint does your bot use for debugging? ` +
            `[Example: http://localhost:3978/api/messages] `, { defaultInput: ' ' });
    }
...
let secret;
if (args.secret) {
    secret = botframework_config_1.BotConfiguration.generateKey();
}
if (!args.name) {
    ...
    if (validurl.isHttpUri(args.endpoint) || validurl.isHttpsUri(args.endpoint)) {
        bot.connectService(new botframework_config_1.EndpointService({
            name: args.name,
