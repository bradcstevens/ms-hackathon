...
async function processUpdateArgs(config) {
    if (args.stdin) {
    ...
    for (const service of config.services) {
        if (service.type === botframework_config_1.ServiceTypes.AppInsights) {
            const appInsights = service;
            if (appInsights.id === args.id || appInsights.serviceName === args.serviceName) {
                ...
                await stdioAsync_1.stdoutAsync(JSON.stringify(appInsights, null, 2));
                return config;
            }
        }
    }
...
//# sourceMappingURL=msbot-update-appinsights.js.map