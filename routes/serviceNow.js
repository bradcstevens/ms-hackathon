...
const createIncident = (dialogData, callerId) => {
    let route = "https://dev59625.service-now.com/api/now/v1/table/incident?sysparm_suppress_auto_sys_field=true";
    let incident = {
        caller_id: callerId,
...
const resolveIncident = (dialogData, callerId) => {
    let route = `https://dev59625.service-now.com/api/now/table/incident/${dialogData.incidentId}?sysparm_exclude_ref_link=true`;
    let resolveIncident = {
        caller_id: callerId,
...
const updateIncident = (dialogData, callerId) => {
    let route = `https://dev59625.service-now.com/api/now/v1/table/task/${dialogData.incidentId}?sysparm_exclude_ref_link=true`;
    let updateIncident = {
        caller_id: callerId,
